...
class Map:
    ...
    def merge(self, indices, points, normals, colors, R, t):
        ...
        '''
        weight = 1 # gaussian weight defined to be 1 in the question.

        # transform from camera to map frame to align with maintained points in map.
        ...
        # print("len indices = ", len(indices), indices[-1])

        # Calculate common denominator
        denom = self.weights[indices,:] + weight

        # Update measurements - points, normals, colors.
        self.points[indices,:]  = (self.weights[indices,:] * self.points[indices,:]  + weight * transformed_points) / denom
        self.normals[indices,:] = (self.weights[indices,:] * self.normals[indices,:] + weight * transformed_normals)/ denom
        self.colors[indices,:]  = (self.weights[indices,:] * self.colors[indices,:]  + weight * colors)/ denom

        # Normalize normals Ha
    ...
    def add(self, points, normals, colors, R, t):
        ...
        # print("Current map points = ", self.points.shape)
        # self.points = np.block([[self.points],[points]])
        # print("Shap after adding  = ", self.points.shape)


        transformed_points  = (R @ points.T + t).T 
        transformed_normals = (R @ normals.T).T 

        self.points  = np.block([[self.points],[transformed_points]])
        self.normals = np.block([[self.normals],[transformed_normals]])
        self.colors  = np.block([[self.colors],[colors]])
        weights      = np.ones((len(points),1))
        self.weights = np.block([[self.weights],[weights]])
    ...
    def filter_pass1(self, us, vs, ds, h, w):
        ...
        #     if ds[i] <=0:
                # print("Error : Depth map value less than zero.")
    ...
    def filter_pass2(self, points, normals, input_points, input_normals,
        ...
        distance_mask = distances < dist_diff
        cosine_angles = np.sum(normals * input_normals,axis = 1)
        angles = np.arccos(cosine_angles)
        angle_mask = angles < angle_diff
