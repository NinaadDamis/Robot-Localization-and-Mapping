...
def find_projective_correspondence(source_points,
    ...
    print("Vertex shape = ", target_vertex_map.shape)
    ...
    print("T_source_point shape = ", T_source_points.shape)
    ...
    print("Target us shape = ", target_us.shape)
 
    # TODO: first filter: valid projection
    mask = np.zeros_like(target_us).astype(bool)
    # target_us[0:20] < w
    mask = (target_us >= 0) & (target_us < w) & (target_vs >=0) & (target_vs < h)
    # mask = np.bitwise_and(target_us >= 0,target_us < w,target_vs >=0,target_vs < h)
    ...
    print("Points remaining after first filter = ", len(source_indices))

    # TODO: second filter: apply distance threshold
    # mask = np.zeros_like(target_us).astype(bool)
    target_correspondences = target_vertex_map[target_vs,target_us]
    print("Target correspondences shape = ", target_correspondences.shape)
    print("T_source_point shape = ", T_source_points.shape)
    distances = np.linalg.norm(T_source_points - target_correspondences,axis = 1)
    print("Norm array shape = ", distances.shape)
    mask =  distances < dist_diff
    # End of TODO

    source_indices = source_indices[mask]
    target_us = target_us[mask]
    target_vs = target_vs[mask]
    print("Points remaining after second filter = ", len(source_indices))
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    print("P_prime transpose shape = ", p_prime.shape)
    q = target_points
    n_q = target_normals
    ...
    for i in range(M):
        ...
        bi       = nx*(px - qx) + ny*(py - qy) + nz*(pz - qz)
        # print("Ai shape = {} , bi shape = {}".format(Ai.shape,bi.shape))
        A[i,:]   = Ai
        b[i]     = -bi 
    # End of TODO
...
def solve(A, b):
    ...
    x, aa,bb,cc = np.linalg.lstsq(A,b)
    # print("SHAPEEEEEEEEEEEE",x.reshape((6,)).shape)
    return x.reshape((6,))
