...
def compute_meas_obs_jacobian(x, i, j, n_poses):
    ...
    d   = np.sqrt(dx**2 + dy**2)
    dsq = dx**2 + dy**2
    # print("d = {} , dsq = {}".format(d,dsq))
    # if(d ==0):
    #     print("i = {} , j = {}".format(i,j))
    jacobian = np.array([[dy/dsq , -dx/dsq, -dy/dsq, dx/dsq ],
                         [-dx/d  , -dy/d  , dx/d   , dy/d ]]) 
...
def create_linear_system(x, odoms, observations, sigma_odom, sigma_observation,
    ...
    for i in range(n_odom): 
        z_i    = odoms[i].reshape((2,))
        h_i    = odometry_estimation(x,i)
        meas_i = z_i - h_i
    ...
    for i in range(n_obs):
        ...
        full_mat = sqrt_inv_obs @ compute_meas_obs_jacobian(x,pose_idx,land_idx,n_poses) # 2 by 4

        h_i    = bearing_range_estimation(x,pose_idx, land_idx, n_poses)
        z_i    = observations[i,2:].reshape((2,)) 
        meas_i = z_i - h_i
        meas_i[0] = warp2pi(meas_i[0])
...
if __name__ == '__main__':
    ...
    for method in args.method:
        ...
        for i in range(10):
            # print("Iteration i = {}/10   ,  x shape = {}".format(i,x.shape))
